...
from sample_integration.uniswap.uniswap_v2.uniswap_v2_pool_model import UniswapV2PoolHook

UniswapV2Helper = UniswapV2HelperHook()
...
def main():
    ...
    # Initialize pool
    pool = UniswapV2PoolHook(test['pool'])

    # Initialize pool with test states
    pool.states.add_state("reserve0", test['states']['reserve0'])
    pool.states.add_state("reserve1", test['states']['reserve1'])
    ...
    # Test quoting logic
    fee_amount, amount_out = pool.get_amount_out(token_in = sell_token, amount_in = sell_amount)
